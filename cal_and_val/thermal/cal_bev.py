...
def veh_init(cyc_file_stem: str, dfs: Dict[str, pd.DataFrame]) -> fsim.Vehicle:
    ...
    # initialize SOC
    vd['pt_type']['BatteryElectricVehicle']['res']['state']['soc'] = \
        dfs[cyc_file_stem]["HVBatt_SOC_CAN4__per"].iloc[1] / 100
    assert 0 < vd['pt_type']['BatteryElectricVehicle']['res']['state']['soc'] < 1, "\ninit soc: {}\nhead: {}".format(
        vd['pt_type']['BatteryElectricVehicle']['res']['state']['soc'], dfs[cyc_file_stem]["HVBatt_SOC_CAN4__per"].head())
    # initialize cabin temp
    vd['cabin']['LumpedCabin']['state']['temperature_kelvin'] = \
        dfs[cyc_file_stem]["Cabin_Lower_Vent_Temp__C"][0] + celsius_to_kelvin_offset
    ...
    vd['pt_type']['BatteryElectricVehicle']['res']['thrml']['RESLumpedThermal']['state']['temperature_kelvin'] = \
        dfs[cyc_file_stem]["Cabin_Lower_Vent_Temp__C"][0] + celsius_to_kelvin_offset
...
for key, df_for_cal in dfs_for_cal.items():
    ...
    # cut out junk data from specific files
    if key == "62009040 Test Data":
        df_for_cal = df_for_cal[df_for_cal["Time[s]_RawFacilities"] < 2153]
...
def get_mod_soc(sd_dict):
    return np.array(sd_dict['veh']['pt_type']['BatteryElectricVehicle']['res']['history']['soc'])
...
em_eff_fwd_max = fsim.ElectricMachine.from_pydict(veh_dict['pt_type']['BatteryElectricVehicle']['em'], skip_init=False).eff_fwd_max 
em_eff_fwd_range = fsim.ElectricMachine.from_pydict(veh_dict['pt_type']['BatteryElectricVehicle']['em'], skip_init=False).eff_fwd_range 
